...
class SpectralPrior(nn.Module):
    def __init__(self, inplanes, planes, bias=True):
        super(SpectralPrior, self).__init__()
        ...
        self.se = SpectralAttention(inplanes, inplanes)
    ...
    def forward(self, x, P, H, time_emb, condition):
        ...
        x = self.se(x)
        x1 = self.conv1(torch.cat((condition, x), dim=1))
        ...
        x2 = self.se(x2)
        x2 = self.conv1(torch.cat((condition, x2), dim=1))
        ...
        x3 = self.se(x3)
        x3 = self.conv1(torch.cat((condition, x3), dim=1))
...
class SpecDataIntegrity(nn.Module):
    def __init__(self):
        super(SpecDataIntegrity, self).__init__()
        self.beta = nn.Parameter(torch.tensor(1.0))
        self.alpha = nn.Parameter(torch.tensor(1.0))
        self.R = nn.Conv2d(hs_channels, ms_channels, 1)
        self.RT = nn.Conv2d(ms_channels, hs_channels, 1)

    def forward(self, H, X, Y):
        IN1 = self.R(H) - Y
        IN2 = (H - X) * self.alpha
        IN = (self.RT(IN1) + IN2) * self.beta
        H_T = H - IN
        return H_T
...
#     ms = torch.randn(1, 4, 160, 160)
#     P0, P1, P2 = torch.randn(1, 102, 160, 160), torch.randn(1, 102, 160, 160), torch.randn(1, 102, 160, 160)
#     model = SpecReconstruction()
#     X0, X1, X2, X3 = model(P0, ms, P0, P1, P2)
